...
type DeliveryStatus =
  'pending' | 'scheduled' | 'preparing' | 'out_for_delivery' | 'delivered' | 'failed' | 'cancelled'
...
const isAdminDashboard = (
  dashboard: AdminDashboardData | CustomerDashboardData
): dashboard is AdminDashboardData => 'metrics' in dashboard
const adminDashboard = computed<AdminDashboardData | null>(() =>
  isAdminDashboard(props.dashboard) ? props.dashboard : null
)
const customerDashboard = computed<CustomerDashboardData | null>(() =>
  isAdminDashboard(props.dashboard) ? null : props.dashboard
)
const salesPoints = computed(() =>
  salesPeriod.value === 'daily'
    ? (adminDashboard.value?.charts.daily_sales ?? [])
    : (adminDashboard.value?.charts.monthly_sales ?? [])
)
...
const formatMetric = (metric: DashboardMetric) =>
  metric.format === 'currency' ? currency.format(metric.value) : number.format(metric.value)
    ...
    <div class="app-page">
      ...
      <template v-if="customerDashboard">
          ...
          >
            <CardHeader class="pb-2"
              ...
              ><CardTitle class="text-3xl">{{
                number.format(customerDashboard.points.balance)
              }}</CardTitle></CardHeader
            >
            <CardContent class="text-sm text-muted-foreground"
              >{{ formatCurrency(Number(customerDashboard.points.peso_equivalent)) }} peso
        ...
        >
          <CardHeader>
            ...
            <CardDescription
              >Fully pay your existing pautang balance before creating another
        ...
        <div class="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader class="flex-row items-start justify-between gap-4 space-y-0">
              <div>
                ...
                ><CardDescription>Your most recently placed unfinished order</CardDescription>
              </div>
              <span
            ...
            <CardContent v-if="customerDashboard.active_order" class="space-y-4">
              <div>
                ...
                <p class="mt-1 text-sm text-muted-foreground">
                  {{ customerDashboard.active_order.rice_product }} ·
                  {{ customerDashboard.active_order.quantity }} sack{{
              ...
              <div class="grid gap-3 text-sm sm:grid-cols-2">
                <div>
                  ...
                  <p class="font-medium">
                    {{ formatCurrency(Number(customerDashboard.active_order.final_amount)) }}
                  </p>
                </div>
                <div>
              ...
              <Button size="sm" variant="outline" as-child
                ><Link :href="route('orders.show', { order: customerDashboard.active_order.id })"
                  >View Order</Link
                ></Button
              >
            </CardContent>
            <CardContent v-else class="space-y-4"
              ><p class="text-sm text-muted-foreground">
                You do not have an unfinished order right now.
              </p>
          ...
          <Card>
            <CardHeader
              ...
              ><CardDescription
                >Your current credit balance and next payment</CardDescription
              ></CardHeader
            >
            <CardContent v-if="customerDashboard.active_pautang" class="space-y-4">
              ...
              <div class="grid gap-3 text-sm sm:grid-cols-3">
                <div>
                  ...
                  <p class="font-medium">
                    {{ formatCurrency(Number(customerDashboard.active_pautang.amount_paid)) }}
                  </p>
                </div>
                <div>
                  <p class="text-muted-foreground">Remaining</p>
                  <p class="font-medium">
                    {{ formatCurrency(Number(customerDashboard.active_pautang.remaining_balance)) }}
                  </p>
                </div>
                <div>
              ...
              <div class="flex flex-col gap-2 sm:flex-row">
                <Button
                  v-if="
                    customerDashboard.active_pautang.can_submit_payment &&
                    customerDashboard.active_pautang.payable_installment_id
                  "
                ...
                ><Button class="w-full sm:w-auto" variant="outline" as-child
                  ><Link
                    :href="route('orders.show', { order: customerDashboard.active_pautang.id })"
                    >View order</Link
                  ></Button
                >
              </div>
            ...
            <CardContent v-else
              ><p class="text-sm text-muted-foreground">
                You do not have an unpaid pautang balance.
              </p></CardContent
            >
          </Card>
        </div>
        ...
        <div class="grid gap-6 xl:grid-cols-3">
          <Card>
            <CardHeader class="flex-row items-center justify-between gap-4 space-y-0"
              ><div>
                ...
                ><CardDescription>Your latest five orders</CardDescription>
              </div>
              <Link class="text-sm text-primary underline" :href="route('orders.index')"
                >View all</Link
              ></CardHeader
            >
            <CardContent class="space-y-3"
              ><p
                v-if="customerDashboard.recent_orders.length === 0"
          ...
          <Card>
            <CardHeader
              ...
              ><CardDescription>Your latest GCash submissions</CardDescription></CardHeader
            >
            <CardContent class="space-y-3"
              ><p
                v-if="customerDashboard.recent_payments.length === 0"
                class="py-4 text-sm text-muted-foreground"
              >
                  ...
                  ><div class="flex items-start justify-between gap-3">
                    <div>
                      <p class="font-medium">
                        ...
                        }}<span v-if="payment.installment_number">
                          · Give {{ payment.installment_number }}</span
                        >
                      </p>
                      <p class="mt-1 text-xs text-muted-foreground">
          ...
          <Card>
            <CardHeader class="flex-row items-center justify-between gap-4 space-y-0"
              ...
              <Link class="text-sm text-primary underline" :href="route('points.show')"
                >View history</Link
              ></CardHeader
            >
            <CardContent class="space-y-3"
              ><p
                v-if="customerDashboard.recent_points_transactions.length === 0"
                class="py-4 text-sm text-muted-foreground"
              >
              ...
              <template v-else
                ><div
                  v-for="entry in customerDashboard.recent_points_transactions"
                  :key="entry.id"
        ...
        <div class="grid gap-6 xl:grid-cols-2">
          <Card>
            <CardHeader class="flex-row items-start justify-between gap-4 space-y-0">
              <div>
                ...
                ><CardDescription>Booked, non-cancelled orders</CardDescription>
              </div>
            ...
            <CardContent>
              <div
                v-if="salesPoints.some((point) => point.value > 0)"
          ...
          <Card>
            <CardHeader
              ...
              ><CardDescription
                >Approved GCash payments over the last 7 days</CardDescription
              ></CardHeader
            >
            <CardContent>
              <div
              ...
              <p v-else class="py-20 text-center text-sm text-muted-foreground">
                No approved collections in this period.
              </p>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader
              ...
              ><CardDescription>Unpaid pautang balances</CardDescription></CardHeader
            >
          ...
          <Card>
            <CardHeader
              ><CardTitle>Best-selling rice</CardTitle
              ><CardDescription>Top five products by ordered sacks</CardDescription></CardHeader
            >
            ...
            <CardContent v-else class="py-12 text-center text-sm text-muted-foreground"
              >No product sales yet.</CardContent
            >
          </Card>
          ...
          <Card>
            ...
            <CardContent class="grid gap-4 sm:grid-cols-2">
              <div
                v-for="point in adminDashboard.charts.payment_timing"
                :key="point.label"
      ...
      <Card v-else
        ><CardHeader
          ...
          ><CardDescription>Please refresh the page and try again.</CardDescription></CardHeader
        ></Card
      >
    </div>
  </AppLayout>
