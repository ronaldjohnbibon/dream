...
const markRead = (notification: CustomerNotification) =>
    router.patch(route('notifications.read', { notification: notification.id }), {}, { preserveScroll: true });
        ...
        <div class="mx-auto w-full max-w-5xl space-y-6 p-4 md:p-6">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">Order, payment, installment, and points updates.</p>
                </div>
                <Button class="w-full sm:w-auto" variant="outline" @click="markAllRead">Mark all as read</Button>
            </div>

            <Card>
                <CardHeader
                    ><CardTitle>Notification history</CardTitle
                    ><CardDescription>Notifications are kept here for your reference.</CardDescription></CardHeader
                >
                <CardContent class="space-y-4">
                    <div class="space-y-3 md:hidden">
                        <p v-if="notifications.data.length === 0" class="py-8 text-center text-sm text-muted-foreground">
                        ...
                        >
                            ...
                            <div class="mt-3 flex items-center justify-between gap-3">
                                ...
                                <Button size="sm" variant="outline" @click="openNotification(notification)">View</Button>
                            </div>
                            <Button v-if="!notification.read_at" class="mt-2" size="sm" variant="ghost" @click="markRead(notification)"
                                >Mark read</Button
                            >
                        </article>
                    </div>
                        ...
                        <DataTable>
                            ...
                            <template #body>
                                <tr v-if="notifications.data.length === 0">
                                    <td colspan="3" class="px-4 py-10 text-center text-muted-foreground">You have no notifications yet.</td>
                                </tr>
                                <tr v-for="notification in notifications.data" :key="notification.id" :class="!notification.read_at && 'bg-muted/40'">
                                    ...
                                    <td class="px-4 py-3 text-right">
                                        <div class="inline-flex gap-2">
                                            <Button size="sm" variant="outline" @click="openNotification(notification)">View</Button
                                            ><Button v-if="!notification.read_at" size="sm" variant="ghost" @click="markRead(notification)"
                                                >Mark read</Button
                                            >
                                        </div>
                                    </td>
