...
import type {
  PaginatedPautangOrders,
  PautangInstallmentStatus,
  PautangSummary,
...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
  { title: props.canManage ? 'Pautang' : 'My pautang', href: route('pautang.index') },
])
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="flex flex-1 flex-col gap-6 p-4 md:p-6">
        ...
        <p class="mt-1 text-sm text-muted-foreground">
          {{
            canManage
              ...
              : 'Track your current two-give payment schedule.'
          }}
        </p>
      </div>
          ...
          <template #body>
            <tr v-if="!pautangOrders || pautangOrders.data.length === 0">
              <td colspan="8" class="px-4 py-10 text-center text-muted-foreground">
                No pautang orders match this view.
              </td>
            </tr>
      ...
      <template v-else-if="pautang">
        <div class="grid gap-4 sm:grid-cols-3">
          <Card
            ><CardHeader class="pb-2"
              ...
              ><CardTitle class="text-2xl">{{
                currency.format(Number(pautang.order_amount))
          ...
          <Card
            ><CardHeader class="pb-2"
              ...
              ><CardTitle class="text-2xl">{{
                currency.format(Number(pautang.amount_paid))
              }}</CardTitle></CardHeader
            ></Card
          >
          <Card
            ><CardHeader class="pb-2"
              ...
              ><CardTitle class="text-2xl">{{
                currency.format(Number(pautang.remaining_balance))
              }}</CardTitle></CardHeader
            ></Card
          >
        </div>
        <Card>
          <CardHeader
            ><CardTitle>Current pautang</CardTitle
            ><CardDescription>Order {{ pautang.order_number }}</CardDescription></CardHeader
          >
          <CardContent class="space-y-4">
            <div
              v-for="installment in pautang.installments"
              :key="installment.id"
            ...
            >
              ...
              <div class="mt-4 grid gap-3 text-sm sm:grid-cols-4">
                ...
                <div>
                  ...
                  <p class="font-medium">
                    {{ currency.format(Number(installment.remaining_balance)) }}
                  </p>
                </div>
              </div>
            </div>
      ...
      <Card v-else>
        <CardHeader
          ...
          ><CardDescription
            >You do not have an active pautang payment schedule.</CardDescription
          ></CardHeader
        >
      </Card>
    </div>
