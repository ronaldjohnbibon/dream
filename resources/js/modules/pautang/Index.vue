        ...
        <div class="flex flex-1 flex-col gap-6 p-4 md:p-6">
            <div>
                ...
                <p class="mt-1 text-sm text-muted-foreground">
                    {{ canManage ? 'Review active, paid, and overdue customer pautang.' : 'Track your current two-give payment schedule.' }}
                </p>
            </div>
            ...
            <template v-if="canManage">
                <div class="flex flex-wrap gap-2">
                    <Button v-for="tab in views" :key="tab" :variant="view === tab ? 'default' : 'outline'" as-child>
                        <Link :href="route('pautang.index', { view: tab })">{{
                            tab === 'paid' ? 'Fully Paid' : tab[0].toUpperCase() + tab.slice(1)
                        }}</Link>
                    </Button>
                </div>
                ...
                <DataTable>
                    ...
                    <template #body>
                        <tr v-if="!pautangOrders || pautangOrders.data.length === 0">
                            <td colspan="8" class="px-4 py-10 text-center text-muted-foreground">No pautang orders match this view.</td>
                        </tr>
                        <tr v-for="order in pautangOrders?.data ?? []" :key="order.id">
                            <td class="px-4 py-3">
                                <p class="font-medium">{{ order.customer?.name }}</p>
                                <p class="text-xs text-muted-foreground">
                                    {{ order.customer?.mobile_number ?? order.customer?.email ?? 'No contact details' }}
                                </p>
                            </td>
                            ...
                            <td class="px-4 py-3">
                                {{ order.days_overdue > 0 ? `${order.days_overdue} day${order.days_overdue === 1 ? '' : 's'}` : '—' }}
                            </td>
                ...
                <div class="flex justify-end text-sm text-muted-foreground">
                    <PaginationLinks v-if="pautangOrders" :links="pautangOrders.links" />
                </div>
            </template>
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    <Card
                        ><CardHeader class="pb-2"
                            ...
                            ><CardTitle class="text-2xl">{{ currency.format(Number(pautang.order_amount)) }}</CardTitle></CardHeader
                        ></Card
                    >
                    <Card
                        ><CardHeader class="pb-2"
                            ><CardDescription>Amount paid</CardDescription
                            ><CardTitle class="text-2xl">{{ currency.format(Number(pautang.amount_paid)) }}</CardTitle></CardHeader
                        ></Card
                    >
                    <Card
                        ><CardHeader class="pb-2"
                            ><CardDescription>Remaining balance</CardDescription
                            ><CardTitle class="text-2xl">{{ currency.format(Number(pautang.remaining_balance)) }}</CardTitle></CardHeader
                        ></Card
                    >
                </div>
                <Card>
                    <CardHeader
                        ><CardTitle>Current pautang</CardTitle><CardDescription>Order {{ pautang.order_number }}</CardDescription></CardHeader
                    >
                    <CardContent class="space-y-4">
                        <div v-for="installment in pautang.installments" :key="installment.id" class="rounded-lg border p-4">
                            <div class="flex flex-wrap items-center justify-between gap-3">
                                ...
                                <span class="rounded-full px-2 py-1 text-xs" :class="installmentStatusClass(installment.status)">{{
                                    installmentStatusLabel(installment.status)
                                }}</span>
                            </div>
                            <div class="mt-4 grid gap-3 text-sm sm:grid-cols-4">
                                <div>
                                    <p class="text-muted-foreground">Amount due</p>
                                    <p class="font-medium">{{ currency.format(Number(installment.amount_due)) }}</p>
                                </div>
                                <div>
                                    <p class="text-muted-foreground">Due date</p>
                                    <p class="font-medium">{{ installment.due_date }}</p>
                                </div>
                                <div>
                                    <p class="text-muted-foreground">Amount paid</p>
                                    <p class="font-medium">{{ currency.format(Number(installment.amount_paid)) }}</p>
                                </div>
                                <div>
                                    <p class="text-muted-foreground">Remaining</p>
                                    <p class="font-medium">{{ currency.format(Number(installment.remaining_balance)) }}</p>
                                </div>
                            </div>
                        </div>
                        <Button class="w-full sm:w-auto" variant="outline" as-child
                            ><Link :href="route('orders.show', { order: pautang.id })">View order</Link></Button
                        >
                    </CardContent>
                </Card>
            </template>
            ...
            <Card v-else>
                <CardHeader
                    ><CardTitle>No current pautang</CardTitle
                    ><CardDescription>You do not have an active pautang payment schedule.</CardDescription></CardHeader
                >
            </Card>
        </div>
    </AppLayout>
