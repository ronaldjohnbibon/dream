...
const breadcrumbs: BreadcrumbItem[] = [
  ...
  {
    title: 'Adjust stock',
    href: route('rice-products.stock.create', { riceProduct: props.product.id }),
  },
]
...
const reducesStock = computed(
  () =>
    form.type === 'order' ||
    form.type === 'damaged' ||
    (form.type === 'adjustment' && form.adjustment_direction === 'decrease')
)
const submit = () =>
  form.post(route('rice-products.stock.store', { riceProduct: props.product.id }))
</script>
      ...
      <Card>
        <CardHeader>
          ...
          <p class="text-sm text-muted-foreground">
            {{ product.name }} currently has {{ product.available_stock }} available sacks.
          </p>
        </CardHeader>
        <CardContent>
