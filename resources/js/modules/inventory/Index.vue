...
const isLowStock = (product: RiceProduct) =>
  product.is_active && product.available_stock <= props.lowStockThreshold
</script>
      ...
      <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        ...
        <div class="flex gap-2">
          <Button variant="outline" as-child
            ><Link :href="route('inventory-movements.index')">Movement history</Link></Button
          >
        ...
        <template #body>
          <tr v-if="products.data.length === 0">
            <td colspan="6" class="px-4 py-10 text-center text-muted-foreground">
              No rice products match the current filters.
            </td>
          </tr>
          <tr v-for="product in products.data" :key="product.id">
            ...
            <td class="px-4 py-3 font-medium" :class="isLowStock(product) && 'text-destructive'">
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex justify-end gap-1">
                <Button size="sm" variant="ghost" as-child
                  ><Link :href="route('rice-products.show', { riceProduct: product.id })"
                    >View</Link
                  ></Button
                >
                <Button size="sm" variant="ghost" as-child
