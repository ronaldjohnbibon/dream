...
const canPayFullyWithPoints = computed(
    () => selectedProduct.value !== null && form.quantity === 1 && maximumPointsToUse.value > 0 && finalAmountWithMaximumPoints.value === 0,
);
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-full max-w-3xl space-y-6 p-4 md:p-6">
            ...
            <Card v-if="products.length === 0">
                ...
                <CardContent
                    ><Button variant="outline" as-child><Link :href="route('orders.index')">Back to my orders</Link></Button></CardContent
                >
            </Card>
            ...
            <form v-else class="space-y-6" @submit.prevent="submit">
                    ...
                    <CardContent class="grid gap-5 sm:grid-cols-2">
                        ...
                        <div v-if="selectedProduct" class="rounded-md border bg-muted/30 p-4 text-sm sm:col-span-2">
                            <div class="flex flex-wrap justify-between gap-2">
                                <span>Unit price</span><span class="font-medium">{{ currency.format(Number(selectedProduct.selling_price)) }}</span>
                            </div>
                            <div class="mt-2 flex flex-wrap justify-between gap-2 text-base">
                                <span class="font-medium">Subtotal</span><span class="font-semibold">{{ currency.format(subtotal) }}</span>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                    ...
                    <CardContent class="space-y-5">
                        ...
                        <div class="grid gap-4 sm:grid-cols-[minmax(0,240px)_1fr] sm:items-end">
                            ...
                            <div class="flex flex-wrap items-center gap-3 text-sm text-muted-foreground">
                                ...
                                <Button v-if="canPayFullyWithPoints" type="button" size="sm" variant="outline" @click="payFullyUsingPoints"
                    ...
                    <CardContent class="grid gap-5 sm:grid-cols-2">
                        <div class="rounded-md border bg-muted/30 p-4 text-sm sm:col-span-2">
                            ...
                            <p class="mt-1 text-muted-foreground">
                                Pautang is limited to {{ pautang.maximum_sacks }} sack(s) and is paid through scheduled installments.
                            </p>
                        </div>
                        <p
                        ...
                        <div class="rounded-md border bg-muted/30 p-4 text-sm sm:col-span-2">
                            <div class="flex justify-between gap-3">
                                <span>Delivery fee</span><span class="font-medium">{{ currency.format(deliveryFee) }}</span>
                            </div>
                            <div class="mt-2 flex justify-between gap-3 text-base">
                                <span class="font-medium">Order total</span><span class="font-semibold">{{ currency.format(finalAmount) }}</span>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                >
                    ...
                    ><Button class="flex-1 sm:flex-none" type="submit" :disabled="form.processing || !pautang.enabled || pautang.has_unpaid_order"
                        >Place order</Button
                    >
                </div>
            </form>
