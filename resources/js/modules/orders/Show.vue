    ...
    <Head :title="order.order_number" /><AppLayout :breadcrumbs="breadcrumbs"
        ><div class="mx-auto w-full max-w-5xl space-y-6 p-4 md:p-6">
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    <div class="flex flex-wrap items-center gap-3">
                        ...
                        <span v-if="order.delivery" class="rounded-full px-2 py-1 text-xs" :class="deliveryClass(order.delivery.status)">{{
                            deliveryStatusLabels[order.delivery.status]
                        }}</span>
                    </div>
                    <p class="mt-1 text-sm text-muted-foreground">Ordered on {{ order.order_date }}</p>
                </div>
                <Button class="w-full sm:w-auto" variant="outline" as-child><Link :href="route('orders.index')">Back to orders</Link></Button>
            </div>
            <div class="grid gap-4 sm:grid-cols-4">
                <Card
                    ><CardHeader class="pb-2"><CardTitle class="text-sm font-medium text-muted-foreground">Rice subtotal</CardTitle></CardHeader
                    ><CardContent
                        ><p class="text-xl font-semibold">{{ currency.format(Number(order.subtotal)) }}</p></CardContent
                    ></Card
                ><Card
                    ><CardHeader class="pb-2"><CardTitle class="text-sm font-medium text-muted-foreground">Delivery fee</CardTitle></CardHeader
                    ><CardContent
                        ><p class="text-xl font-semibold">{{ currency.format(Number(order.delivery_fee)) }}</p></CardContent
                    ></Card
                ><Card
                    ><CardHeader class="pb-2"><CardTitle class="text-sm font-medium text-muted-foreground">Final amount</CardTitle></CardHeader
                    ><CardContent
                        ><p class="text-xl font-semibold">{{ currency.format(Number(order.final_amount)) }}</p></CardContent
                    ></Card
                ><Card
                    ><CardHeader class="pb-2"><CardTitle class="text-sm font-medium text-muted-foreground">Payment status</CardTitle></CardHeader
                    ><CardContent
                        ><p class="font-semibold">Pautang</p>
                        <p class="mt-1 text-sm text-muted-foreground">{{ paymentStatusLabels[order.payment_status] }}</p></CardContent
                    ></Card
                >
            </div>
            <Card v-if="order.delivery"
                ...
                ><CardContent class="grid gap-5 text-sm sm:grid-cols-2"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <p class="mt-1 whitespace-pre-line font-medium">{{ order.delivery.delivery_address }}</p>
                    </div>
                    ...
                    <div v-if="canManage" class="sm:col-span-2">
                        <Button as-child><Link :href="route('deliveries.show', { delivery: order.delivery.id })">Manage delivery</Link></Button>
                    </div></CardContent
                ></Card
            >
                ...
                ><CardContent class="space-y-3"
                    ><div v-for="installment in order.pautang_installments" :key="installment.id" class="rounded-lg border p-4">
                        ...
                        <Button
                            ...
                            as-child
                            ><Link :href="route('gcash-payments.create', { order: order.id, installment: installment.id })"
            ...
            <Card v-if="order.gcash_payments.length"
                ...
                ><CardContent class="space-y-3"
                    ><div v-for="payment in order.gcash_payments" :key="payment.id" class="flex justify-between gap-3 rounded-lg border p-4">
                        <div class="text-sm">
                            <p class="font-medium">
                                {{ currency.format(Number(payment.amount)) }}
                                <span class="ml-2 rounded-full px-2 py-1 text-xs" :class="paymentClass(payment.status)">{{
                        ...
                        <a :href="payment.screenshot_url" target="_blank" class="text-sm text-primary underline">View screenshot</a>
                    </div></CardContent
                ></Card
            >
        </div></AppLayout
