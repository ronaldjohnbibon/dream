...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs"
        ><div class="flex flex-1 flex-col gap-6 p-4 md:p-6">
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">
                        {{ canManage ? 'Review customer orders and delivery progress.' : 'Track your rice orders and delivery progress.' }}
                    </p>
                </div>
                <Button v-if="!canManage" class="w-full sm:w-auto" as-child><Link :href="route('orders.create')">Order rice</Link></Button>
            </div>
            <form
                class="grid gap-3 rounded-lg border bg-card p-4 md:grid-cols-[1fr_repeat(2,180px)_auto]"
                @submit.prevent="router.get(route('orders.index'), filters, { preserveState: true, replace: true })"
            >
                ...
                >
                    ...
                    <option v-for="status in statuses" :key="status" :value="status">{{ deliveryStatusLabels[status] }}</option></select
                ><select v-model="filters.payment_status" class="h-9 rounded-md border border-input bg-background px-3 text-sm">
                    <option value="all">All payment statuses</option>
                    <option v-for="status in paymentStatuses" :key="status" :value="status">{{ paymentStatusLabels[status] }}</option></select
                ><Button type="submit" variant="outline">Apply filters</Button>
            </form>
            <div v-if="!canManage" class="space-y-3 md:hidden">
                <Card v-if="orders.data.length === 0"
                    ><CardContent class="py-10 text-center text-sm text-muted-foreground">No orders match the current filters.</CardContent></Card
                >
                <Link
                ...
                >
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="mt-1 text-sm text-muted-foreground">
                                {{ order.rice_product.name }} · {{ order.quantity }} sack{{ order.quantity === 1 ? '' : 's' }}
                            </p>
                        </div>
                        <p class="text-right font-semibold">{{ currency.format(Number(order.final_amount)) }}</p>
                    </div>
                    <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
                        <span class="rounded-full bg-muted px-2 py-1">{{ paymentStatusLabels[order.payment_status] }}</span
                        ><span v-if="order.delivery" class="rounded-full px-2 py-1" :class="statusClass(order.delivery.status)">{{
                        ...
                        <tr v-for="order in orders.data" :key="order.id">
                            ...
                            <td class="px-4 py-3">
                                ...
                                <p class="text-xs text-muted-foreground">
                                    {{ order.rice_product.brand }} · {{ order.quantity }} sack{{ order.quantity === 1 ? '' : 's' }}
                                </p>
                            </td>
                            <td class="px-4 py-3 font-medium">{{ currency.format(Number(order.final_amount)) }}</td>
                            <td class="px-4 py-3">
                                <p>Pautang</p>
                                <p class="text-xs text-muted-foreground">{{ paymentStatusLabels[order.payment_status] }}</p>
                            </td>
                            <td class="px-4 py-3">
                                <span v-if="order.delivery" class="rounded-full px-2 py-1 text-xs" :class="statusClass(order.delivery.status)">{{
                                    deliveryStatusLabels[order.delivery.status]
                                }}</span>
                            </td>
            ...
            <div class="flex justify-end text-sm text-muted-foreground"><PaginationLinks :links="orders.links" /></div></div
    ></AppLayout>
</template>
