    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-full max-w-3xl space-y-6 p-4 md:p-6">
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                ...
                <Card>
                    <CardHeader><CardTitle>Payment details</CardTitle><CardDescription>Give {{ installment.installment_number }} · Remaining {{ currency.format(Number(installment.remaining_balance)) }}</CardDescription></CardHeader>
                    <CardContent>
                        <form class="space-y-5" @submit.prevent="submit">
                            <FormField id="amount" label="Amount" :error="form.errors.amount" required>
                                <input id="amount" v-model="form.amount" type="number" min="0.01" :max="paymentLimit" step="0.01" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" required />
                            </FormField>
