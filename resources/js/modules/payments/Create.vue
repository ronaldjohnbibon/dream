...
const props = defineProps<{
    ...
    installment: { id: number; installment_number: number; amount_due: string; remaining_balance: string };
            ...
            <div class="grid gap-6 lg:grid-cols-2">
                <Card>
                    <CardHeader
                        ...
                        ><CardDescription
                            >Send to {{ gcash.account_name }} · {{ gcash.account_number }}, then upload your proof of payment.</CardDescription
                        ></CardHeader
                    >
                    <CardContent
                        ><img :src="gcash.qr_code_url" alt="GCash payment QR code" class="mx-auto max-h-96 rounded-lg border object-contain"
                    /></CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <form class="space-y-5" @submit.prevent="submit">
                            ...
                            <FormField id="reference-number" label="GCash reference number" :error="form.errors.reference_number" required
                                ><input
                                    id="reference-number"
                                    ...
                                    required
                            /></FormField>
                            <FormField id="payment-date" label="Payment date" :error="form.errors.payment_date" required
                                ><input
                                    id="payment-date"
                                    ...
                                    required
                            /></FormField>
                            <FormField id="screenshot" label="Payment screenshot" :error="form.errors.screenshot" required
                                ><input
                                    id="screenshot"
                                    type="file"
                                    ...
                                    required
                                    @change="form.screenshot = ($event.target as HTMLInputElement).files?.[0] ?? null"
                            /></FormField>
                            ...
                            >
                                <Button class="w-full sm:w-auto" type="submit" :disabled="form.processing">Submit for verification</Button>
                            </div>
                        </form>
                    </CardContent>
                </Card>
