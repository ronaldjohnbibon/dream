...
const breadcrumbs = computed<BreadcrumbItem[]>(() => [
  { title: 'Payments', href: route('gcash-payments.index') },
  {
    title: `Payment #${props.payment.id}`,
    href: route('gcash-payments.show', { gcashPayment: props.payment.id }),
  },
])
...
const approve = () =>
  approveForm.patch(route('gcash-payments.approve', { gcashPayment: props.payment.id }))
const reject = () =>
  rejectForm.patch(route('gcash-payments.reject', { gcashPayment: props.payment.id }))
</script>
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="mx-auto w-full max-w-5xl space-y-6 p-4 md:p-6">
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold tracking-tight">GCash payment #{{ payment.id }}</h1>
          <p class="mt-1 text-sm text-muted-foreground">
            {{ gcashPaymentStatusLabels[payment.status] }}
          </p>
        </div>
        <Button variant="outline" as-child
          ><Link :href="route('gcash-payments.index')">Back to payments</Link></Button
        >
      </div>
        ...
        <Card
          ...
          ><CardContent class="space-y-4"
            ><img
              ...
              class="max-h-[36rem] w-full rounded-lg border object-contain"
            /><a
          ...
          <Card
            ...
            ><CardContent class="grid gap-4 text-sm sm:grid-cols-2"
              ><div>
                ...
                <p>{{ payment.customer.mobile_number ?? payment.customer.email }}</p>
              </div>
              <div>
                <p class="text-muted-foreground">Order</p>
                <p class="font-medium">{{ payment.order.order_number }}</p>
                <p v-if="payment.installment">Give {{ payment.installment.installment_number }}</p>
              </div>
              <div>
                <p class="text-muted-foreground">Amount</p>
                <p class="font-medium">{{ currency.format(Number(payment.amount)) }}</p>
              </div>
              <div>
              ...
              <div class="sm:col-span-2">
                <p class="text-muted-foreground">GCash reference number</p>
                <p class="font-mono font-medium">{{ payment.reference_number }}</p>
              </div></CardContent
            ></Card
          >
          <Card v-if="pending"
            ><CardHeader><CardTitle>Review payment</CardTitle></CardHeader
            ><CardContent class="grid gap-6 md:grid-cols-2"
              ...
              <form class="space-y-4" @submit.prevent="reject">
                <FormField
                  ...
                  :error="rejectForm.errors.remarks"
                  required
                >
                  <textarea
                    id="reject-remarks"
          ...
          <Card v-else
            ...
            ><CardContent class="text-sm"
              ...
              <p class="mt-3 whitespace-pre-line">
                <span class="text-muted-foreground">Remarks:</span> {{ payment.remarks ?? 'None' }}
              </p></CardContent
            ></Card
          >
        </div>
