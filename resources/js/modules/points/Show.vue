        ...
        <div class="mx-auto w-full max-w-5xl space-y-6 p-4 md:p-6">
            ...
            <div class="grid gap-4 sm:grid-cols-3">
                <Card
                    ><CardHeader class="pb-2"
                        ><CardDescription>Current points</CardDescription><CardTitle class="text-3xl">{{ balance }}</CardTitle></CardHeader
                    ></Card
                >
                <Card
                    ><CardHeader class="pb-2"
                        ><CardDescription>Peso equivalent</CardDescription
                        ><CardTitle class="text-3xl">{{ currency.format(Number(pesoEquivalent)) }}</CardTitle></CardHeader
                    ></Card
                >
                <Card
                    ><CardHeader class="pb-2"
                        ><CardDescription>Conversion rate</CardDescription
                        ><CardTitle class="text-3xl">{{ currency.format(Number(pesoPerPoint)) }}</CardTitle></CardHeader
            ...
            <Card v-if="canAdjust">
                <CardHeader
                    ...
                    ><CardDescription
                        >Enter a positive value to add points or a negative value to deduct them. The balance cannot go below zero.</CardDescription
                    ></CardHeader
                >
                <CardContent>
            ...
            <Card>
                <CardHeader
                    ...
                    ><CardDescription>Every earned, redeemed, or adjusted point is listed here.</CardDescription></CardHeader
                >
                <CardContent class="space-y-4">
                    <div v-if="!canAdjust" class="space-y-3 md:hidden">
                        <p v-if="ledger.data.length === 0" class="py-8 text-center text-sm text-muted-foreground">No points transactions yet.</p>
                        <div v-for="entry in ledger.data" :key="entry.id" class="rounded-lg border p-4">
                            ...
                            <p v-if="entry.order" class="mt-2 text-xs text-muted-foreground">Order {{ entry.order.order_number }}</p>
                        </div>
                    </div>
