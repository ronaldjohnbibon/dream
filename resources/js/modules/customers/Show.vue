        ...
        <div class="mx-auto w-full max-w-6xl space-y-6 p-4 md:p-6">
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                ...
                <div class="flex flex-wrap gap-2">
                    <Button v-if="customer.account_status !== 'suspended'" variant="destructive" @click="suspendDialogOpen = true"
            ...
            <Card>
                ...
                <CardContent class="grid gap-5 text-sm sm:grid-cols-2">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <p class="mt-1 whitespace-pre-line font-medium">{{ customer.complete_address || 'Not provided' }}</p>
                    </div>
                    <div>
                        <p class="text-muted-foreground">Created</p>
                        <p class="mt-1 font-medium">{{ formattedDate.format(new Date(customer.created_at)) }}</p>
                    </div>
                    <div>
                        <p class="text-muted-foreground">Last updated</p>
                        <p class="mt-1 font-medium">{{ formattedDate.format(new Date(customer.updated_at)) }}</p>
                    </div>
                </CardContent>
            </Card>

            <section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4" aria-label="Customer account summary">
                <Card
                ...
                <Card
                    ><CardHeader class="pb-2"
                        ...
                        ><CardTitle class="text-2xl">{{ currency.format(Number(summary.peso_equivalent)) }}</CardTitle></CardHeader
                    ></Card
                >
                <Card
                    ><CardHeader class="pb-2"
                        ><CardDescription>Total orders</CardDescription><CardTitle class="text-2xl">{{ summary.total_orders }}</CardTitle></CardHeader
                    ></Card
                >
                <Card
                    ><CardHeader class="pb-2"
                        ><CardDescription>Outstanding balance</CardDescription
                        ><CardTitle class="text-2xl">{{ currency.format(summary.outstanding_balance) }}</CardTitle></CardHeader
                    ></Card
                >
                <Card
                    ><CardHeader class="pb-2"
            ...
            <Card>
                <CardHeader
                    ...
                    ><CardDescription
                        >Add a positive amount or enter a negative amount to deduct points. The balance cannot go below zero.</CardDescription
                    ></CardHeader
                >
                <CardContent>
                    <form class="grid gap-5 sm:grid-cols-[180px_1fr_auto] sm:items-end" @submit.prevent="submitAdjustment">
                        <FormField id="points-change" label="Points change" :error="adjustment.errors.points" required
                            ><Input id="points-change" v-model.number="adjustment.points" type="number" step="1" required
                        /></FormField>
                        <FormField id="adjustment-reason" label="Reason" :error="adjustment.errors.reason" required
            ...
            <Card>
                <CardHeader><CardTitle>Order history</CardTitle><CardDescription>All orders placed by this customer.</CardDescription></CardHeader>
                    ...
                    <DataTable>
                        ...
                        <template #body>
                            <tr v-if="orders.data.length === 0">
                                <td colspan="7" class="px-4 py-10 text-center text-muted-foreground">No orders yet.</td>
                            </tr>
                            <tr v-for="order in orders.data" :key="order.id">
                                <td class="px-4 py-3 font-medium">
                                    <Link class="hover:underline" :href="route('orders.show', { order: order.id })">{{ order.order_number }}</Link>
                                </td>
                                <td class="px-4 py-3 text-muted-foreground">{{ dateFormatter.format(new Date(`${order.order_date}T00:00:00`)) }}</td>
                                <td class="px-4 py-3">
                                ...
                                <td class="px-4 py-3 text-right font-medium">{{ currency.format(Number(order.final_amount)) }}</td>
                                <td class="px-4 py-3 text-right">{{ currency.format(Number(order.remaining_balance)) }}</td>
                            </tr>
                        </template>
                    </DataTable>
            ...
            <Card>
                <CardHeader
                    ...
                    ><CardDescription>All submitted GCash payments, including pending and rejected records.</CardDescription></CardHeader
                >
                <CardContent class="space-y-4">
                    <DataTable>
                        <template #head
                        ...
                        <template #body>
                            <tr v-if="payments.data.length === 0">
                                <td colspan="6" class="px-4 py-10 text-center text-muted-foreground">No payment submissions yet.</td>
                            </tr>
                            <tr v-for="payment in payments.data" :key="payment.id">
                                ...
                                <td class="px-4 py-3 font-medium">
                                    <Link class="hover:underline" :href="route('gcash-payments.show', { gcashPayment: payment.id })">{{
                                ...
                                <td class="px-4 py-3">
                                    <span class="rounded-full px-2 py-1 text-xs" :class="paymentReviewStatusClass(payment.status)">{{
            ...
            <Card>
                <CardHeader
                    ><CardTitle>Points history</CardTitle><CardDescription>Every earned, redeemed, or adjusted point.</CardDescription></CardHeader
                >
                <CardContent class="space-y-4">
                    <DataTable>
                        <template #head
                        ...
                        <template #body>
                            <tr v-if="pointsHistory.data.length === 0">
                                <td colspan="5" class="px-4 py-10 text-center text-muted-foreground">No points transactions yet.</td>
                            </tr>
                            <tr v-for="entry in pointsHistory.data" :key="entry.id">
                                ...
                                <td class="px-4 py-3">
                                    <Link v-if="entry.order" class="hover:underline" :href="route('orders.show', { order: entry.order.id })">{{
