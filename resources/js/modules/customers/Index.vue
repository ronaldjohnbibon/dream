...
import type { Customer, CustomerFilters, CustomerStatus, PaginatedCustomers } from '@/modules/customers/types';
...
const sortIndicator = (sort: CustomerFilters['sort']) => props.filters.sort === sort ? (props.filters.direction === 'asc' ? ' ↑' : ' ↓') : '';
        ...
        <div class="flex flex-1 flex-col gap-6 p-4 md:p-6">
            <div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p class="mt-1 text-sm text-muted-foreground">Manage customer profiles and account status.</p>
                </div>
            ...
            <form class="grid gap-3 rounded-lg border bg-card p-4 md:grid-cols-[1fr_180px_auto]" @submit.prevent="applyFilters">
                <Input v-model="filters.search" placeholder="Search name, email, or mobile number" aria-label="Search customers" />
            ...
            <DataTable>
                <template #head>
                    <tr>
                        ...
                        <th class="px-4 py-3 font-medium"><button type="button" @click="sortBy('mobile_number')">Mobile{{ sortIndicator('mobile_number') }}</button></th>
                ...
                <template #body>
                    <tr v-if="customers.data.length === 0">
                        <td colspan="7" class="px-4 py-10 text-center text-muted-foreground">No customers match the current filters.</td>
                    </tr>
                    <tr v-for="customer in customers.data" :key="customer.id">
                        ...
                        <td class="px-4 py-3"><span class="rounded-full px-2 py-1 text-xs" :class="statusClass(customer.account_status)">{{ statusLabel(customer.account_status) }}</span></td>
                        <td class="px-4 py-3 text-muted-foreground">{{ dateFormatter.format(new Date(customer.created_at)) }}</td>
                        <td class="px-4 py-3 text-right">
                            <div class="flex justify-end gap-2">
                                <Button variant="ghost" size="sm" as-child><Link :href="route('customers.show', { customer: customer.id })">View</Link></Button>
                                <Button variant="ghost" size="sm" as-child><Link :href="route('customers.edit', { customer: customer.id })">Edit</Link></Button>
                            </div>
                        </td>
                    </tr>
                </template>
