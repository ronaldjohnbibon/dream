...
<template>
    <form class="space-y-6" @submit.prevent="emit('submit')">
        ...
        <FormField id="mobile_number" label="Mobile number" :error="form.errors.mobile_number" required>
            <Input id="mobile_number" v-model="form.mobile_number" type="tel" autocomplete="tel" required />
        </FormField>

        <FormField id="complete_address" label="Complete address" :error="form.errors.complete_address" required>
            <textarea id="complete_address" v-model="form.complete_address" rows="3" class="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm outline-none focus-visible:border-ring focus-visible:ring-1 focus-visible:ring-ring" required />
        </FormField>
