...
const deleteDescription = computed(() =>
  selectedUser.value
    ? `This permanently deletes ${selectedUser.value.name}. This action cannot be undone.`
    : ''
)
...
const sortIndicator = (sort: UserFilters['sort']) =>
            ...
            <td class="px-4 py-3 text-right">
              <div class="flex justify-end gap-2">
                <Button variant="ghost" size="sm" as-child
                  ><Link :href="route('users.show', { user: user.id })">View</Link></Button
                >
                <Button variant="ghost" size="sm" as-child
                  ><Link :href="route('users.edit', { user: user.id })">Edit</Link></Button
                >
                <Button
                  variant="ghost"
                  size="sm"
