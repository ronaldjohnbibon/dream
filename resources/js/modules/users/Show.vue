    ...
    <div class="mx-auto w-full max-w-2xl p-4 md:p-6">
      <div class="mb-4 flex items-center justify-between gap-4">
        <div>
          ...
          <p class="text-sm text-muted-foreground">Administrator details</p>
        </div>
        <Button as-child
          ><Link :href="route('users.edit', { user: user.id })">Edit administrator</Link></Button
        >
      </div>
      ...
      <Card>
        ...
        <CardContent class="grid gap-5 text-sm sm:grid-cols-2">
          ...
          <div>
            ...
            <p class="mt-1 font-medium">Administrator</p>
          </div>
          <div>
            <p class="text-muted-foreground">Created</p>
            <p class="mt-1 font-medium">{{ formattedDate.format(new Date(user.created_at)) }}</p>
          </div>
          <div>
            <p class="text-muted-foreground">Last updated</p>
            <p class="mt-1 font-medium">{{ formattedDate.format(new Date(user.updated_at)) }}</p>
          </div>
        </CardContent>
      </Card>
    </div>
