...
const isActive = (href: string) =>
    page.url === href || page.url.startsWith(`${href}?`) || (href !== route('dashboard') && page.url.startsWith(`${href}/`));
        ...
        <ul class="mx-auto grid max-w-lg grid-cols-5 gap-1">
            <li v-for="item in items" :key="item.title">
                <Link
                    ...
                    :class="isActive(item.href) ? 'bg-primary text-primary-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'"
