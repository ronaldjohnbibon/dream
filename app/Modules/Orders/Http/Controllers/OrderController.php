...
class OrderController extends Controller
{
    ...
    public function store(StoreOrderRequest $request): RedirectResponse
    {
        ...
        $order = DB::transaction(function () use ($attributes, $request, &$redeemedPoints, &$pointsLog, &$created): Order {
            ...
            if (! $product->is_active || $product->available_stock < $quantity) {
                throw ValidationException::withMessages(['quantity' => 'There is not enough stock available for this order.']);
            }
            $system = SystemSetting::current();
            if (! $system->pautang_enabled) {
                throw ValidationException::withMessages(['order' => 'Pautang is not currently available.']);
            }
            if ($quantity > $system->pautang_max_sacks) {
                throw ValidationException::withMessages(['quantity' => "Pautang orders are limited to {$system->pautang_max_sacks} sack(s)."]);
            }
    ...
    private function pointsDiscount(User $customer, int $points, float $subtotal): float
    {
        ...
        if ($points > $this->points->currentBalance($customer)) {
            throw ValidationException::withMessages(['points_to_use' => 'You cannot use more points than your available balance.']);
        }
        $settings = $this->points->settings();
        if (! $settings->is_enabled) {
            throw ValidationException::withMessages(['points_to_use' => 'Points redemption is not currently available.']);
        }
        if ($points < $settings->minimum_redemption) {
            throw ValidationException::withMessages(['points_to_use' => "Use at least {$settings->minimum_redemption} points per order."]);
        }
        if ($points > $settings->maximum_points_usable) {
            throw ValidationException::withMessages(['points_to_use' => "You can use at most {$settings->maximum_points_usable} points per order."]);
        }
        $rate = (float) $settings->peso_per_point;
        if ($rate <= 0) {
            throw ValidationException::withMessages(['points_to_use' => 'Points redemption is not currently available.']);
        }
        if ($points > (int) floor(($subtotal + 0.000001) / $rate)) {
            throw ValidationException::withMessages(['points_to_use' => 'Points used cannot reduce the rice subtotal below zero.']);
        }
    ...
    private function pautangPaymentStatus($installments): string
    {
        ...
        if ($installments->every(fn (PautangInstallment $item) => (float) $item->remaining_balance <= 0)) {
            return 'paid';
        }
        if ($installments->contains(fn (PautangInstallment $item) => $item->currentStatus() === 'overdue')) {
            return 'overdue';
        }
